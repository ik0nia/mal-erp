...
class LowStockSupplierWidget extends Widget
{
    protected string $view = 'filament.app.widgets.low-stock-supplier';
    ...
    private function loadData(): void
    {
        ...
        $query = DB::table('product_suppliers as ps')
            ...
            ->whereRaw('(COALESCE(stk.total_qty, 0) / bpv.avg_out_qty_7d) < 14')
            ->select([
                'wp.id as product_id',
                'wp.sku',
                'wp.name',
                ...
                DB::raw('COALESCE(stk.total_qty, 0) / bpv.avg_out_qty_7d as days_to_stockout'),
            ]);
        ...
        $results = $query
            ->orderByRaw('days_to_stockout ASC')
            ->limit(100)
            ->get();
