    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Action::make('print_awb')
                ...
                ->color('primary')
                ->visible(fn (): bool => \App\Models\SamedayAwb::where('woo_order_id', $this->record->id)
                    ->whereNotNull('awb_number')->where('awb_number', '!=', '')
                    ->whereNotIn('status', [\App\Models\SamedayAwb::STATUS_CANCELLED, \App\Models\SamedayAwb::STATUS_FAILED])
                    ->exists())
                ->action(function () {
                    $awb = \App\Models\SamedayAwb::where('woo_order_id', $this->record->id)
                        ->whereNotNull('awb_number')->where('awb_number', '!=', '')->latest('id')->first();
                    try {
                        $connection = $awb->connection ?? \App\Models\IntegrationConnection::find($awb->integration_connection_id);
                        $pdf = app(\App\Services\Courier\SamedayAwbService::class)->downloadAwbPdf($connection, $awb->awb_number);

                        return response()->streamDownload(
                            fn () => print($pdf),
                            'AWB-'.$awb->awb_number.'.pdf',
            ...
            Action::make('awb_status')
                ->label('Status livrare')
                ->icon('heroicon-o-map-pin')
                ->color('gray')
                ->visible(fn (): bool => \App\Models\SamedayAwb::where('woo_order_id', $this->record->id)
                    ->whereNotNull('awb_number')->where('awb_number', '!=', '')->exists())
                ->action(function (): void {
                    $awb = \App\Models\SamedayAwb::where('woo_order_id', $this->record->id)
                        ->whereNotNull('awb_number')->where('awb_number', '!=', '')->latest('id')->first();
                    try {
                        $connection = $awb->connection ?? \App\Models\IntegrationConnection::find($awb->integration_connection_id);
                        $tracking = app(\App\Services\Courier\SamedayAwbService::class)->getAwbStatusHistory($connection, $awb->awb_number);
